...
class JobboardModelUnsolicited extends JModel
{
	...
	function getData()
	{
		...
		if(empty($this->data))
		{
			...
			$app->setUserState('com_jobboard.unsolicited.count',$this->count);
		}
	...
	function getSearch()
	{
		if(!$this->_search)
		{
			...
			$search = $app->getUserStateFromRequest("com_jobboard.unsolicited.search", 'search', '', 'string');
	...
	function _buildQuery()
	{
		if(!$this->_query)
		{
			...
			$this->_query = "SELECT *
                        FROM
                            #__jobboard_unsolicited";
			
			if($search != '')
			{
				$fields = array('first_name','email','title');
				$where = array();
