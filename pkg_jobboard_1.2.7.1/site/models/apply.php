...
class JobboardModelApply extends JModel
{
     ...
     */
     ...
     function getQuestionnaire($qid) {
             $db = & $this->getDBO();
             $sql = 'SELECT `name`, `title`, `description`, `fields`  FROM  `#__jobboard_questionnaires`
                     WHERE `qid` = '.$qid;  
             $db->setQuery($sql);
             return $db->loadAssoc();
     }
     ...
     */
     ...
     /**
     * Save questionnaire
     * @params questionnaire id , user id, application id, data
     *
     * @return boolean
     */

     function saveQuestionnaire($qid, $uid, $aid, $data) {
             $db = & $this->getDBO();
             $sql = 'INSERT INTO `#__jobboard_q'.$qid.'` (';
             $sql .= '`uid`, `appl_id`, ';
    ...
    function getUsrApplicationsForJob($jid) {
           $db = & $this->getDBO();
           $sql = 'SELECT * FROM `#__jobboard_usr_applications`
                        WHERE `job_id` = '.$jid;
           $db->setQuery($sql);
           return $db->loadAssocList();
    }
