...
class JobboardModelAdmapplicationslist extends JModel
{
	var $_data = null;
    var $_db = null;
	var $_total = null;
    ...
    function _buildQuery()
	{
		...
		{
  		    if($uid <> 0 && $jid <> 0)
                  $where =  'WHERE a.`job_id`='.$jid.' AND a.`user_id` <> '.$uid;
            else $where = ' '; 
	...
	function getData()
	{
		if(empty($this->_data))
		{
            ...
            $app = & JFactory::getApplication();
		    $app->setUserState('com.jobboard.admin.applist.count', $count);
		}
	   return $this->_data;
	}
