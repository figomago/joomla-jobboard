...
class JobboardControllerTaglist extends JController
{
    /**
    * constructor
    */
        ...
        $address_coords; $address_components; $address_string = ''; $address_data = array();
         ...
         if(!empty($remote_result) && is_array($remote_result)) {
           if($remote_result['status'] == 'OK') {
               ...
               $app->setUserState("com_jobboard.tl.geo_address", $address_string, 'string');
           }
         }
      }
    ...
    private function _showList($selcat=1, $reset_keywds=false)
	{
        ...
        if(!empty($r_srch) || !empty($r_srchloc)) JRequest::checkToken() or jexit('Invalid Token');
        if(!empty($r_srchkey)) :
           $r_srchkey_get = JRequest::getVar('keysrch', '', 'get');
           if(empty($r_srchkey_get))  // check token only if keyword submitted via form
        ...
        if(strlen($keysrch) == 0) return $this->setRedirect(JRoute::_('index.php?option=com_jobboard&view=list'));
    ...
    function resetListKeywords() {
        $app = JFactory::getApplication();
  	    ...
  	    $selcat = $app->getUserStateFromRequest('com_jobboard.tl.list.selcat', 'selcat', 1, 'int');
        $this->_showList($selcat, true);
    }
}

$controller = new JobboardControllerTaglist();
