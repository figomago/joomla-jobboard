...
class JobboardModelTaglist extends JModel
{
	var $_total = null;
	...
	function __construct()
	{
    	...
    	$filter_job_type = $app->getUserState("com_jobboard.tl.filter_job_type",  array(), 'array');
    	$filter_careerlvl = $app->getUserState("com_jobboard.tl.filter_careerlvl",  array(), 'array');
    	$filter_edulevel = $app->getUserState("com_jobboard.tl.filter_edulevel",  array(), 'array');
	...
	function _buildQuery()
	{
		...
		$category_id = $app->getUserState('com_jobboard.tl.list.selcat', 1, 'int');
		$country_id = $app->getUserState('com_jobboard.tl.list.country_id', 0, 'int');
        ...
        $loc_cfg = $app->getUserState('com_jobboard.tl.use_location', 0);
		...
		$sort = $app->getUserState('com_jobboard.tl.list.sort','sort','');
		switch ($sort)
		{
			case 'a':
				$direction = ' ASC ';
		...
		$order = $app->getUserState('com_jobboard.tl.list.order', '');
		...
		switch ($order)
		{
			...
			case 'distance':
                if($locsrch == '') :
				    $order_type = ', j.`city`'.$direction;
                    $app->setUserState('com_jobboard.tl.list.order', 'city');
                else :
		...
		$layout = $app->getUserState('com_jobboard.tl.list.layout', 'list', 'string');
        ...
        if($locsrch != '' && $loc_cfg == 1) { // search by location
              ...
              $sel_distance = $app->getUserState("com_jobboard.tl.sel_distance",  50, 'int');
              $geo_coords = $app->getUserState("com_jobboard.tl.geo_coords", array(), 'array');
	...
	function getData()
	{
		...
		if(empty($this->data))
		{
			...
			$app->setUserState('com_jobboard.tl.list.count',$this->count);
		}
		return $this->data;
	}
	...
	function getSearch()
	{
		if(!$this->_search)
		{
			...
			$search = $app->getUserState("com_jobboard.tl.jobsearch", '', 'string');
	...
	function getKeySearch()
	{
		if(!$this->_keysrch)
		{
			...
			$keysrch = $app->getUserState("com_jobboard.tl.keysrch", '', 'string');
	...
	function getLocSearch()
	{
		if(!$this->_locsrch)
		{
			...
			$locsrch = $app->getUserState("com_jobboard.tl.locsrch", '', 'string');
